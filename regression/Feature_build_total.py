...
bg_pois = bg_pois.merge(smart_loc[['BGFIPS', 'D3B', 'Pct_AO0', 'Pct_AO1', 'Pct_AO2p', 'NatWalkInd', 'D3A', 'D1C', 'D4C',
                                   'CBSA', 'CBSA_POP', 'Ac_Total']], on='BGFIPS', how='left')
...
g_pois = g_pois.merge(poi_ty, on='naics_code', how='left')
g_pois_need = g_pois.merge(CT_Features[['BGFIPS', 'Population_Density', 'Median_income', 'White_Non_Hispanic_R',
                                        'Rural_Population_R', 'Household_Below_Poverty_R']], on='BGFIPS')
g_pois_need.to_pickle(r'D:\Google_Review\Parking\temp\pois_parking_with_socioinfo.pkl')

# How many CBG in each MSA: Remove those small MSA
...
print('No of MSA: %s' % len(ct_cbg.loc[ct_cbg['BGFIPS'] > 40, 'NAMELSAD']))
...
(MSA_geo[(MSA_geo['LSAD'] == 'M1') & (MSA_geo['CBSAFP'].isin(bg_msa['CBSA']))]
 .geometry.boundary.plot(color=None, edgecolor='k', linewidth=1, alpha=0.8, ax=ax))
ax.tick_params(left=False, labelleft=False, bottom=False, labelbottom=False)
ax.axis('off')
plt.tight_layout()
plt.savefig(r'D:\Google_Review\Parking\results\spatial_sentiment_us_new.png', dpi=1000)

# Plot boxplot: Avg sentiment by MSA: Top and Last 10
...
# MSA correlation with socio factors
all_corr = pd.DataFrame()
for csa in set(bg_msa['CBSA'].dropna()):
    temp = bg_pois[(bg_pois['CBSA'] == csa)]
    temp['weight_st'] = temp['weight_st'].astype(float)
    corr_t = temp.corr(numeric_only=True)[['weight_st']]
    corr_t.columns = [csa]
    corr_t = corr_t.reset_index()
...
pty_corr = all_corr[all_corr.index.isin(des_mean_tt['index'])].T.unstack().reset_index()
pty_corr.columns = ['Variable', 'CBSA', 'Corr']
pty_corr = pty_corr.merge(msa_population)
pty_corr = pty_corr.sort_values(by='CBSA_POP', ascending=False)
pty_corr = pty_corr[pty_corr['Variable'] == 'Population_Density']
# pty_corr = pty_corr[pty_corr['CBSA_POP'] > np.percentile(pty_corr['CBSA_POP'], 50)]

# Plot spatial map: choose by Correlation
for kk in ['39740', '35620', '26420', '16980']:
    # kk = '26420'
    msa_t_geo = bg_pois_geo[(bg_pois_geo['CBSA'] == kk)]
    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(9, 6))
    msa_t_geo.plot(column='weight_st', ax=ax, legend=False, scheme='user_defined',
                   classification_kwds={'bins': [-0.5, 0, 0.5]}, cmap='bwr',
                   legend_kwds=dict(frameon=False, ncol=4, loc='lower center', bbox_to_anchor=(0.5, 0)), linewidth=0.1,
                   edgecolor='k')
    MSA_geo[(MSA_geo['CBSAFP'] == kk)].geometry.boundary.plot(color=None, edgecolor='k', linewidth=0.5, alpha=0.8,
                                                              ax=ax)
    ax.tick_params(left=False, labelleft=False, bottom=False, labelbottom=False)
    ax.axis('off')
    plt.tight_layout()
    plt.savefig(r'D:\Google_Review\Parking\results\spatial_sentiment_%s_new.png' % kk, dpi=1000)
    plt.close()
